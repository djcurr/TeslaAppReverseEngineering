...
public class GMSSRootCalc {
    ...
    public GMSSRootCalc(Digest digest, byte[][] bArr, int[] iArr, Treehash[] treehashArr, Vector[] vectorArr) {
        ...
        for (int i13 = 0; i13 < this.heightOfTree; i13++) {
            this.index[i13] = iArr[i13 + 8];
        }
        this.heightOfNodes = new Vector();
        for (int i14 = 0; i14 < i12; i14++) {
            this.heightOfNodes.addElement(Integers.valueOf(iArr[this.heightOfTree + 8 + i14]));
        }
        this.root = bArr[0];
        this.AuthPath = (byte[][]) Array.newInstance(byte.class, this.heightOfTree, this.mdLength);
        int i15 = 0;
        while (i15 < this.heightOfTree) {
            int i16 = i15 + 1;
            this.AuthPath[i15] = bArr[i16];
            i15 = i16;
        }
        this.tailStack = new Vector();
        for (int i17 = 0; i17 < i12; i17++) {
            this.tailStack.addElement(bArr[this.heightOfTree + 1 + i17]);
        }
    ...
    public void initialize(Vector vector) {
        ...
        this.index = new int[i11];
        this.AuthPath = (byte[][]) Array.newInstance(byte.class, i11, this.mdLength);
